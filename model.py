...
class SqueezeExcitation(nn.Module):
    ...
    def forward(self, x):
        ...
        # Excitation step: FC -> ReLU -> FC -> Sigmoid
        y = self.relu(self.fc1(y))
        y = self.sigmoid(self.fc2(y))

        # Reshape to (B, C, 1, 1) and scale the original input
        y = y.view(b, c, 1, 1)
        return x * y
...
class Model(nn.Module):
    ...
    def init_weights(self):
        ...
        nn.init.uniform_(self.pos_embed.weight, -initrange, initrange)
        nn.init.uniform_(self.decoder.weight, -initrange, initrange)

    def forward(self, inputs):
        # inputs: (B, 8, 8) with integer indices from encode_board
        B = inputs.shape[0]

        # Embed pieces
        piece_embeds = self.embedder(inputs)  # shape: (B, 8, 8, embed_dim)

        # Create a positional index for each cell in the board
        # We'll flatten the board to a shape (B, 64)
        flat_positions = torch.arange(64, device=inputs.device).unsqueeze(0).expand(B, -1)
        pos_embeds = self.pos_embed(flat_positions)  # shape: (B, 64, embed_dim)

        # Flatten piece embeddings to match positions
        piece_embeds_flat = piece_embeds.view(B, 64, self.embed_dim)

        # Combine the embeddings: addition is the simplest approach
        combined_embeds = piece_embeds_flat + pos_embeds

        # Reshape back to (B, 8, 8, embed_dim)
        combined_embeds = combined_embeds.view(B, 8, 8, self.embed_dim)

        # Permute to (B, embed_dim, 8, 8) for conv layers
        combined_embeds = combined_embeds.permute(0, 3, 1, 2).contiguous()

        # Pass through conv/attention layers
        x = self.convnet(combined_embeds)

        # Accumulate spatial features
        x = F.relu(self.accumulator(x).squeeze())

        # Decode final score
        scores = self.decoder(x).flatten()
        return scores
