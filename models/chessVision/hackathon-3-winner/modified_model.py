...
class Attention(nn.Module):
    '''
    Implements a temporal attention block with support for one or two attention heads.
    
    n_heads: 1 or 2
    activation: softmax (default), can be changed if needed
    '''
    def __init__(self, input_dims, attention_dims, n_heads=2):
        ...
        # Add a LayerNorm layer to normalize over the feature dimension.
        # We'll apply it after we get the projected output. The normalized_shape 
        # should match the dimension of the features you're normalizing (i.e., input_dims).
    ...
    def forward(self, x):
        ...
        x: shape (B, D, W, H)
            B: Batch size
            D: Number of filters/features (input_dims)
            W, H: Spatial dimensions
        '''
        oB, oD, oW, oH = x.shape
        
        # Flatten spatial dimensions
        x = x.permute(0, 2, 3, 1)  # (B, W, H, D)
        x = x.view(oB, -1, oD)     # (B, W*H, D)
        ...
        # Project the multihead output back to input_dims
        multihead_concat = self.attention_head_projection(multihead)  # shape: (B, W*H, input_dims)

        # Reshape back to original spatial dimensions
        multihead_concat = multihead_concat.view(oB, oW, oH, oD).permute(0, 3, 1, 2)  # (B, D, W, H)

        # Apply LayerNorm
        # LayerNorm expects the feature dimension last, so we'll permute before and after.
        multihead_concat = multihead_concat.permute(0, 2, 3, 1)  # (B, W, H, D)
        multihead_concat = self.layernorm(multihead_concat)      # Normalize over the D dimension
        multihead_concat = multihead_concat.permute(0, 3, 1, 2)  # (B, D, W, H)

        return multihead_concat
...
class Model(nn.Module):
    ...
    def forward(self, inputs):
        ...
        inputs = F.gelu(self.accumulator(inputs).squeeze())
